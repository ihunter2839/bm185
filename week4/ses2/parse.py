...
geneFields += ["num_exons", "length", "product"]
		...
		while i < len(record.features):
			...
			if i == 0:
				...
				for exon in exonList:
					exonData = ""
					exonData += str(gene_count_global) + '\t'
					exonData += str(exon_count_global) + '\t'
					left = exon.split(':')[0]
					exonData += left + '\t'
					right = exon.split(':')[1]
					exonData += right + '\t'
					exonData += str(int(right)-int(left))
					exon_count_global += 1
